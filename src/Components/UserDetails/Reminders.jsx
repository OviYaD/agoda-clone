...
function Reminders({ remainder }) {
    ...
    const [inputCheck, setInputChecked] = useState(remainder)

    const handleChange = () => {
        dispatch({ type: "FETCH_START" })
        axios.post(`${process.env.REACT_APP_API_KEY}/subscription/remainder`, {remainder : !inputCheck}, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
            .then((res) => {
    ...
    return (
        <section className='section'>
            <h2>I would like to receive booking assist reminders</h2>
            <div>
